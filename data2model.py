...
for i in range(0, 183):
    d.append(np.datetime64(pd.to_datetime(rank_to_date[date_rank[i]], format='%Y%m%d')))
def APtoDF():
    ap_dfs.clear()
...
for i in range(0, 244):
    d2.append(np.datetime64(pd.to_datetime(rank_to_date[date_rank2[i]], format='%Y%m%d')))
def stl(j):
    plt.close('all')
    ap_df[j].plot()
    orig = np.log(ap_df[j])
